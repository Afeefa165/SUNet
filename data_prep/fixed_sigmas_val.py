#!/usr/bin/env python3
"""
Fixed-sigma multiple-noise generator
Equivalent to your second MATLAB block (sigmas 10,30,50).
"""
import os
import argparse
import cv2
...
def main(src_dir, out_input_dir, out_target_dir, patch_size=256, crops_per_image=3, seed=1234):
    ...
    for file in tqdm(files, desc='Images'):
        ...
        for _ in range(crops_per_image):
            ...
            for sigma in (10, 30, 50):
                noisy = add_gaussian_noise(C, sigma)
                count += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--src_dir", required=True, help="Path to GT images (source)")
    ...
    main(args.src_dir, args.out_input_dir, args.out_target_dir, args.patch_size, args.crops_per_image)
